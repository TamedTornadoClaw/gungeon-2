...
describe('MiniBossShieldGun mesh scaffold', () => {
  it('MeshId.MiniBossShieldGun exists in the enum', () => {
    expect(MeshId.MiniBossShieldGun).toBeDefined();
    expect(typeof MeshId.MiniBossShieldGun).toBe('number');
  });

  it('produces a valid THREE.Mesh', () => {
    const mesh = manager.acquireMesh(MeshId.MiniBossShieldGun);
    expect(mesh).toBeInstanceOf(THREE.Mesh);
    expect(mesh.geometry).toBeInstanceOf(THREE.BufferGeometry);
    expect(mesh.material).toBeDefined();
    manager.releaseMesh(MeshId.MiniBossShieldGun, mesh);
  });
  ...
  it('has dark green color (0x118833)', () => {
    const mesh = manager.acquireMesh(MeshId.MiniBossShieldGun);
    const mat = mesh.material as THREE.MeshToonMaterial;
    expect(mat.color.getHex()).toBe(0x118833);
    manager.releaseMesh(MeshId.MiniBossShieldGun, mesh);
  });
  ...
  it('getMeshDef returns a valid definition', () => {
    const def = getMeshDef(MeshId.MiniBossShieldGun);
